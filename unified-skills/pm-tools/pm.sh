...
sync_safe() {
  ...
  "$ROOT/scripts/roadmap/sync_roadmap.sh" "$ROADMAP_PATH" "$GH_REPO"
}
...
capture_sessions() {
  ...
  TS=$(date +%F)
  OUT="$ROOT/sessions/${TS}-Pilot005-Excerpt.md"
  python3 "$ROOT/scripts/sessions/sanitize_session.py" --in "${FILES[@]}" --out "$OUT" --max-lines "$MAX_LINES" --title "Pilot 005 — Redacted Session Excerpt" || true
